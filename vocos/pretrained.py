...
class Vocos(nn.Module):
    ...
    def from_hparams(cls, config_path: str) -> Vocos:
        ...
        return cls._instantiate_from_dict(config)

    @classmethod
    def from_config(cls, config_path: str) -> Vocos:
        with open(config_path, "r") as f:
            config = yaml.safe_load(f)
        inner_config = config["model"]["init_args"]
        return cls._instantiate_from_dict(inner_config)

    @classmethod
    def _instantiate_from_dict(cls, config: Dict[str, Any]) -> Vocos:
    ...
    def forward(self, audio_input: torch.Tensor, **kwargs: Any) -> torch.Tensor:
        ...
        """
        if hasattr(self.feature_extractor, "infer"):
            features = self.feature_extractor.infer(audio_input, **kwargs)
        else:
