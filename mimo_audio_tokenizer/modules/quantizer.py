...
def kmeans(samples, num_clusters: int, num_iters: int = 10):
    ...
    for _ in range(num_iters):
        ...
        dists = -(samples.pow(2).sum(1, keepdim=True) - 2 *
                  samples @ means.t() + means.t().pow(2).sum(0, keepdim=True))

        buckets = dists.max(dim=-1).indices
        bins = torch.bincount(buckets, minlength=num_clusters)

        new_means = buckets.new_zeros(num_clusters, dim, dtype=dtype)
        new_means = new_means.scatter_add_(0, repeat(buckets,
                                                     "n -> n d",
                                                     d=dim), samples)
    ...
    def __init__(self, *, num_quantizers, codebook_size, **kwargs):
        ...
        if isinstance(codebook_size, int):
            ...
            codebook_size += [codebook_size[-1]
                              ] * (num_quantizers - len(codebook_size))
...
class ResidualVectorQuantizer(nn.Module):
    ...
    def forward(self,
        ...
        quantized, codes, commit_loss, quantized_list = self.vq(x,
                                                                n_q=n_q,
                                                                layers=layers)
    ...
    def encode(self,
        ...
        Args:
            x (torch.Tensor): Input tensor, (packed_len, d_model).
            n_q (int): Number of quantizer used to quantize. Default: All quantizers.
            st (int): Start to encode input from which layers. Default: 0.
        Returns:
            torch.Tensor: Indices for each quantizer, (n_q, packed_len).
        """
        n_q = n_q if n_q else self.n_q
        st = st or 0
    ...
    def decode(self, codes: torch.Tensor, st: int = 0) -> torch.Tensor:
        ...
        Args:
            codes (torch.Tensor): Input indices for each quantizer, (n_q, packed_len).
            st (int): Start to decode input codes from which layers. Default: 0.
        Returns:
            torch.Tensor: Decoded hidden states, (packed_len, d_model).
        """
